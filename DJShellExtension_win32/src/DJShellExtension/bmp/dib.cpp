...
LPBYTE ConvertDIBFormat( LPBITMAPINFO lpSrcDIB, UINT nWidth, UINT nHeight, UINT nbpp, BOOL bStretch )
{
    ...
    lpbmi = (BITMAPINFO*)malloc( dwTargetHeaderSize );
...
*
\****************************************************************************/
LPBYTE ReadBMPFile( IStream* stream )
{
    ...
    WORD                wPaletteSize = 0;
    DWORD            	dwBitsSize = 0;
    wchar_t* szFileName = L"";
    // Read the header
    if (stream->Read(&bfh,sizeof(BITMAPFILEHEADER),&dwBytes) != S_OK || ( dwBytes != sizeof( BITMAPFILEHEADER ) ) ) {
        MessageBoxW( NULL, L"Error reading bmp header!", szFileName, MB_OK );
        return NULL;
    }
    // Does it look like a BMP file?
    if( ( bfh.bfType != 0x4d42 ) || (bfh.bfReserved1!=0) || (bfh.bfReserved2!=0) )
    {
        MessageBox( NULL, L"Not a recognised BMP format file", szFileName, MB_OK );
        return NULL;
    }
    // Allocate some memory
    if( (lpDIB = (LPBYTE)malloc( sizeof( BITMAPINFO ) )) == NULL )
    {
        MessageBox( NULL, L"Failed to allocate memory for DIB", szFileName, MB_OK );
        return NULL;
    }
    // Read in the BITMAPINFOHEADER    
    if (stream->Read(lpDIB,sizeof(BITMAPINFOHEADER),&dwBytes) != S_OK || ( dwBytes != sizeof( BITMAPINFOHEADER ) ) )
    {
        free( lpDIB );
    ...
    {
        free( lpDIB );
        MessageBox( NULL, L"OS/2 style BMPs Not Supported", szFileName, MB_OK );
        return NULL;
    }
    ...
    if( wPaletteSize != 0 )
    {
        if (stream->Read(((LPBITMAPINFO)lpDIB)->bmiColors, wPaletteSize, &dwBytes) != S_OK || ( dwBytes != wPaletteSize ) ) 
        {
            free( lpDIB );
            MessageBox( NULL, L"Error reading file", szFileName, MB_OK );
            return NULL;
    ...
    {
        LARGE_INTEGER li;
        li.QuadPart = bfh.bfOffBits;
        if (stream->Seek(li,STREAM_SEEK_SET,NULL) != S_OK) 
        {
            free( lpDIB );
            MessageBox( NULL, L"Error reading file", szFileName, MB_OK );
            return NULL;
    ...
    // Read the image bits
    if (stream->Read(FindDIBBits((LPSTR)lpDIB), dwBitsSize, &dwBytes) != S_OK || ( dwBytes != dwBitsSize ) ) 
    {
        free( lpDIB );
        MessageBox( NULL, L"Error reading file", szFileName, MB_OK );
        return NULL;
